...
public class CollectionRepository : GenericRepository<Collection>, ICollectionRepository
{
    ...
    public override async Task<List<Collection>> GetAllAsync()
    {
        var collections = await DbSet.Where(x => x.Status == CollectionStatus.Active.ToString())
            .Include(x => x.Account)
            .Where(c => c.Account.Role != Role.Competitor.ToString() || c.Account == null)
    ...
    {
        var collections = await DbSet.Include(x => x.PaintingCollection).ThenInclude(x => x.Painting).Where(x =>
                x.CreatedBy == accountId && x.Status == CollectionStatus.Active.ToString())
