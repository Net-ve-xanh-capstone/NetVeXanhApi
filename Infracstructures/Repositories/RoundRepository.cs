...
public class RoundRepository : GenericRepository<Round>, IRoundRepository
{
    ...
    {
        return await DbSet.Include(src => src.EducationalLevel).Include(src => src.Schedule).Where(src => src.EducationalLevelId == levelId && src.Status != RoundStatus.Delete.ToString())
            .ToListAsync();
    }
    
    public async Task<List<Round>> GetRoundByContestId(Guid id)
    {
        var list = await DbSet.Include(src => src.EducationalLevel).ThenInclude(src => src.Contest).Include(src => src.Schedule).ThenInclude(src => src.Account).Where(src => src.EducationalLevel.ContestId == id && src.Status != RoundStatus.Delete.ToString())
            .ToListAsync();
        return list;
    }
    ...
    public virtual async Task<bool> CheckSubmitValidDate(Guid? roundId)
    {
        ...
        bool check = DateTime.Now <= temp.EndTime && DateTime.Now >= temp.StartTime && temp.Status == RoundStatus.InProcess.ToString();
        return check;
    }
    ...
    {
        return await DbSet.Where(src => src.EndTime <= DateTime.Now && src.Status == RoundStatus.InProcess.ToString()).ToListAsync();

    }

    public async Task<List<Round>> StartRound()
    {
        return await DbSet.Where(src => src.StartTime >= DateTime.Now && src.Status == RoundStatus.NotStarted.ToString()).ToListAsync();
    }
}