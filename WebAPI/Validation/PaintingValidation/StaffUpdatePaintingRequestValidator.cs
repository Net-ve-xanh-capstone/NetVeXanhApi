    ...
    public class StaffUpdatePaintingRequestValidator : AbstractValidator<StaffUpdatePaintingRequest>
    {
        ...
        public StaffUpdatePaintingRequestValidator(IValidationServiceManager validationServiceManager)
        {
            ...
            RuleFor(user => user.Email)
                ...
                {
                    var currentUser = await _validationServiceManager.AccountValidationService.GetAccountByPaintingId(request.Id);

                    // Kiểm tra nếu số điện thoại không thay đổi
                    if (currentUser.Email == email)
                    {
                        return true; // Số điện thoại không thay đổi, không cần kiểm tra trong DB
                    }
