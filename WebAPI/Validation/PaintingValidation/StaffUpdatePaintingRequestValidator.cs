...
public class StaffUpdatePaintingRequestValidator : AbstractValidator<StaffUpdatePaintingSendModel>
{
    ...
    public StaffUpdatePaintingRequestValidator(IValidationServiceManager validationServiceManager)
    {
        ...
        RuleFor(user => user.Email)
            ...
            {
                var currentUser =
                    await _validationServiceManager.AccountValidationService.GetAccountByPaintingId(request.Id);

                // Kiểm tra nếu số điện thoại không thay đổi
                if (currentUser.Email ==
                    email) return true; // Số điện thoại không thay đổi, không cần kiểm tra trong DB
        ...
        RuleFor(user => user.Phone)
            ...
            .MustAsync(async (request, phone, cancellation) =>
            {
                ...
                if (currentUser.Phone ==
                    phone) return true; // Số điện thoại không thay đổi, không cần kiểm tra trong DB
